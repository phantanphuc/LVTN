...
\title[Nhận dạng biểu thức toán học]{Luận văn tốt nghiệp đại học }
...
\begin{document}
	\begin{frame}[plain]
		\maketitle
		\small
		{\centering\itshape \huge{\bf{Nhận dạng biểu thức toán học}} \par}
		\footnotesize
		\begin{table}[c]
			\centering 
			\begin{tabular}{rrl}
	...
	\begin{frame}{Phạm vi đề tài}
		\begin{itemize}
			\uncover<1->{\item Nhận dạng biểu thức toán học dạng offline\footnote{Nhận dạng từ hình ảnh.}.}
			
			\uncover<2->{\item Mỗi ảnh dùng để nhận dạng chỉ chứa đúng 1 biểu thức và biểu thức này có đường cơ sở\footnote{Thuật ngữ tiếng Anh: baseline} không bị lệch khỏi đường nằm ngang của ảnh.}
			\uncover<3->{\item Chuyển biểu thức từ dạng hình ảnh sang dạng Latex.}
		\end{itemize}
	\end{frame}
	%---------------------------------------------------
	...
	%\subsection{Quy trình nhận dạng biểu thức toán học}
	\begin{frame}{Giới thiệu quy trình nhận dạng}
	\begin{center}
		\centering
		\includegraphics[width=0.99\linewidth]{2phase}
		\vspace{0.5cm}
		\captionof{figure}{Hai quá trình căn bản của nhận dạng biểu thức toán học.}
	\end{center}
	\end{frame}
	...
	\begin{center}
		\centering
		\includegraphics[width=0.99\linewidth]{tuantu}
		\vspace{0.5cm}
		\captionof{figure}{Hai quá trình căn bản của nhận dạng biểu thức toán học.}
	\end{center}
	\end{frame}
	...
	\begin{center}
		\centering
		\includegraphics[width=0.8\linewidth]{kethop}
		\vspace{0.5cm}
		\captionof{figure}{Hai quá trình căn bản của nhận dạng biểu thức toán học.}
	\end{center}
	\end{frame}
	...
	\begin{frame}{Watch, Attend and Parser và Context- aware Recognition}
		\begin{block}{Đánh giá}
			\begin{itemize}
				\item Ưu điểm: tận dụng được thông tin cấu trúc 2 chiều của biểu thức toán học.
				\item Cách làm: đưa cả ảnh biểu thức qua quá trình nhận dạng.
			\end{itemize}
...
%			Cả 2 phương pháp đều tận dụng được thông tin cấu trúc 2 chiều của biểu thức toán học qua quá trình nhận dạng bằng cách sử dụng một lớp mạng CNN để tạo ra đặc trưng từ ảnh biểu thức đầu vào, thay vì sử dụng cách phương pháp phổ biến trong phân tách ký tự như phân tích hình chiếu\footnote{Thuật ngữ tiếng Anh: projection cutting}, phân tích thành phần liên thông\footnote{Thuật ngữ tiếng Anh: connected component analysis}. Do đó, hạn chế lỗi do quá trình nhận dạng ký tự gây ra ảnh hướng đến kết quả sau cùng của bài toán.
	...
	\begin{center}
		\centering
		\includegraphics[width=0.65\linewidth]{overview_1}
		\vspace{0.5cm}
		\captionof{figure}{Tổng quan phương pháp.}
	\end{center}
	\end{frame}
	...
	\begin{frame}
		...
		\begin{itemize}
			\item Mạng SSD\footfullcite{liu2016ssd} - Single Shot Multibox Detector
			\item DRACULAE\footfullcite{zanibbi} - Diagram Recognition Application for Computer Understanding of Large Algebraic Expressions
		\end{itemize}
	...
	\begin{frame}
		...
		\begin{center}
			\centering
			\includegraphics[width=0.95\linewidth]{ssd.png}
			\vspace{0.5cm}
			\captionof{figure}{Mô hình mạng SSD nhóm đề xuất}
		\end{center}
	\end{frame}
	...
	\begin{frame}
		...
		\begin{center}
			\centering
			\includegraphics[width=0.95\linewidth]{ssd2.png}
			\vspace{0.5cm}
			\captionof{figure}{Mô hình mạng SSD nhóm đề xuất}
		\end{center}
	\end{frame}
...
\begin{frame}
	...
	\begin{center}
		\centering
		\includegraphics[width=0.95\linewidth]{ssd3.png}
		\vspace{0.5cm}
		\captionof{figure}{Mô hình bộ phân tích cú pháp Draculae nhóm đề xuất}
	\end{center}
\end{frame}
...
\begin{frame}
	...
	\begin{center}
		\centering
		\includegraphics[width=0.95\linewidth]{r_pipeline.png}
		\vspace{0.5cm}
...
%	\end{frame}
	...
	\begin{frame}
		...
		\begin{block}{Sinh Default Box}
			\begin{itemize}
				\item Các default box được sinh với nhiều mức kích thước khác nhau dựa trên công thức $s_k = s_{min} + \frac{s_{max} - s_{min}}{m - 1}(k - 1)$ .
...
%	\end{frame}
	...
	\begin{frame}
		...
		\begin{block}{Mô hình SSD300}
			\begin{itemize}
				\item Mạng cơ sở: VGG
				\item Số lớp: 106
				\item Kích thước ảnh đầu vào: $300 \times 300$
			\end{itemize}
		\end{block}
	\end{frame}
	...
	\begin{frame}
		...
		\begin{center}
			...
			\captionof{figure}{Mô hình mạng SSD nguyên thủy}
				\begin{block}{Mô hình SSD300}
				\begin{itemize}
					\item Do kích thước default box không đủ nhỏ nên hệ thống gặp khó khăn trong việc nhận diện ký tự nhỏ.
				\end{itemize}
			\end{block}
		\end{center}
	...
	\begin{frame}
		...
		\begin{center}
			\centering
			\includegraphics[width=0.70\linewidth]{im_5.png}
			\vspace{0.5cm}
		...
		\begin{block}{Các thay đổi}
			...
			Điều này khiến lưới default box bị phá hỏng.
		\end{block}
	\end{frame}
	...
	\begin{frame}
		...
		\end{center}
	\begin{block}{Các thay đổi}
		Tăng kích thước ảnh đầu vào từ $300 \times 300$ lên $500 \times 500$ kéo theo kích thước các feature map cũng tăng lên.
	\end{block}
	\end{frame}
	...
	\begin{frame}
		...
		\begin{center}
			\centering
			\includegraphics[width=0.85\linewidth]{im_6.png}
			\vspace{0.5cm}
			\captionof{figure}{Mô hình phiên bản III}
		\end{center}
	\begin{block}{}

		Khi kích thước feature map tăng lên, số lượng default box tương ứng với feature map đó cũng tăng lên góp phần lấp đầy phần lưới bị hổng.
	\end{block}
	\end{frame}
	...
	\begin{block}{Các thay đổi}
		\begin{itemize}
			\item Tăng thêm một khối gồm 02 lớp tích chập nhằm tăng số feature map trích xuất được.
		\end{itemize}
	...
	\begin{frame}
		\frametitle{Sinh cây BST - Baseline Structure Tree}
		\begin{center}
			\centering
...
%	\end{frame}
	...
	\begin{frame}
		...
		\begin{center}
			\centering
			\includegraphics[width=0.9\linewidth]{nhom.png}
			\vspace{0.5cm}
			\captionof{figure}{Sinh cây Lexed BST}
		\end{center}
		
		\begin{block}{Sinh cây Lexed BST}
			\begin{itemize}
				\item Các ký tự sẽ được nhóm lại phù hợp
				\item Sử dụng tập luật để xây dựng ngữ nghĩa.
			\end{itemize}
		\end{block}
	...
	\begin{frame}{Nhắc lại 4 phiên bản thử nghiệm}
		\begin{center}
			\begin{tabular}{||c | c | c | c | c||} 
				...
				\hline
				\makecell{Kích thước \\default boxes\footnote{Kích thước default box được quy định bởi bộ 2 thông số: ($s_{min}$, $s_{max}$)\\}}& (20,90) & (8,50) & (8,50)& (8,50)\\ 
				\hline
	...
	\begin{frame}{Đánh giá định tính}
		\begin{center}
			\centering
			\includegraphics[width=0.95\linewidth]{perfect}
			\vspace{0.5cm}
			\captionof{figure}{Một số ảnh kết quả với những biểu thức lồng nhau sau quá trình nhận dạng của phiên bản IV.}
		\end{center}
	\end{frame}
	...
	\begin{frame}{Ưu điểm}
		\begin{itemize}
			...
			\item Nhận diện được nhiều dạng biểu thức, đặc biệt biểu thức lồng ghép nhau như tích phân nhiều lớp, tổng nhiều lớp,...
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Nhược điểm}
		...
		%	\item Khi có nhiều ký tự trong ảnh thì hệ thống nhận diện gặp nhiều khó khăn.
			\item Vẫn còn sai sót trong việc gán nhãn ký tự.
			\item Tốc độ xử lý một ảnh còn chậm.
			\item Bộ phân tích cú pháp đơn giản.
			\item Các ký tự overlap nhau dễ bị bỏ sót (dấu căn).
		\end{itemize}
	\end{frame}
	\subsection{Những điều đã làm được}
	\begin{frame}{ Những điều đã làm được}
		\begin{itemize}
			\item Xây dựng tập dữ liệu ký tự và biểu thức.
			\item Cải tiến mạng SSD gốc để phát hiện và nhận dạng được những đối tượng có kích thước nhỏ, cụ thể là ký tự toán học viết tay.
			\item Hiện thực bộ parser dựa trên lý thuyết của Zanibbi\footfullcite{zanibbi}.
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Khắc phục lỗi nhận dạng sai ở ký tự lớn}
		\begin{center}
			\centering
			\includegraphics[width=0.775\linewidth]{compare_future.png}
			\vspace{0.2cm}
			\captionof{figure}{Kết quả thu được sau khi thu nhỏ nội dung ảnh.}
		\end{center}
		\begin{block}{Hướng thực hiện}
			Xác định kích thước ký tự như thế nào là tối ưu với hệ thống nhận dạng, từ đó cải tiến bộ tiền xử lý để tạo ra ảnh đầu vào phù hợp với hệ thống.
		\end{block}
	\end{frame}
